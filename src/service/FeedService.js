...
export default class FeedService extends ApiService {
  static normalizeMixedFeedItems(data) {
    ...
    return [
      ...(Array.isArray(data?.posts) ? data.posts.map((post, index) => ({ type: "POST", post, _debugIndex: index })) : []),
      ...(Array.isArray(data?.postList) ? data.postList.map((post, index) => ({ type: "POST", post, _debugIndex: index })) : []),
      ...(Array.isArray(data?.serviceAds) ? data.serviceAds.map((serviceAd, index) => ({ type: "SERVICE_AD", serviceAd, _debugIndex: index })) : []),
      ...(Array.isArray(data?.products) ? data.products.map((product, index) => ({ type: "PRODUCT_RECOMMENDATION", product, _debugIndex: index })) : []),
      ...(Array.isArray(data?.productRecommendations)
        ? data.productRecommendations.map((product, index) => ({
            type: "PRODUCT_RECOMMENDATION",
            product,
  ...
  static async getMixedFeed({
    ...
    return {
      ...response.data,
      items: normalizedItems,
      nextCursor: response.data?.hasMore
        ? {
            cursorCreatedAt: response.data?.nextCursorCreatedAt || null,
            cursorId: response.data?.nextCursorId || null,
          }
        : null,
    };
  }
