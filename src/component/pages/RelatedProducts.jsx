...
const RelatedProducts = ({ searchTerm }) => {
  ...
  useEffect(() => {
    const fetchRelatedProducts = async () => {
      if (!searchTerm) {
        console.log("No search term provided. Skipping related products fetch.");
        setLoading(false);
        return;
      }

      try {
        const data = await ApiService.getRelatedProducts(searchTerm);
        setRelatedProducts(data);
      } catch (err) {
        setError(err.message);
        setAlertMessage("Failed to fetch related products");
        setIsError(true);
        setShowAlert(true);
      } finally {
        setLoading(false);
  ...
  return (
    <Container fluid className={`p-0 ${styles.homeApplianceContainer}`}>
      {/* Alert for messages */}
      <Snackbar
        open={showAlert}
      ...
      <div className={styles.homeApplianceHeader}>
        <h1 className="text-center mb-0 py-2">Related Products</h1>
      </div>
          ...
          {relatedProducts.map((product) => {
            ...
            return (
              <Col
                ...
                xl={2} // 5 products per row on extra-large screens
              ...
              >
                <div className={`card h-100 shadow-sm position-relative ${styles.productCard}`}>
                  ...
                  >
                    <div className={`ratio ratio-1x1 ${styles.productImageContainer}`}>
                      <img
                        src={imageUrl}
                        alt={product.name}
                        className={`card-img-top p-2 object-fit-cover ${styles.productImage}`}
                  ...
                  <div className={`card-body d-flex flex-column p-2 ${styles.productDetails}`}>
                    ...
                    {/* Old Price */}
                    {product.oldPrice > 0 && (
                      <p className={`text-danger text-decoration-line-through mb-1 small ${styles.oldPrice}`}>
                        ₦{product.oldPrice.toLocaleString()}
                      </p>
                    )}

                    {/* New Price */}
                    <p className={`h6 mb-1 text-primary ${styles.newPrice}`}>
