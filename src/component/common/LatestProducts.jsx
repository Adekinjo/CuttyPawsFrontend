...
import styles from "../../style/LatestProducts.module.css"; // Ensure this CSS file exists
...
const LatestProducts = () => {
  ...
  useEffect(() => {
    const fetchLatestProducts = async () => {
      try {
        ...
        // Sort products by date added (assuming each product has a `createdAt` field)
        const sortedProducts = allProducts.sort(
          (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
        );
        // Get the last 12 products
        ...
        setAlertMessage("Unable to fetch latest products");
        setIsError(true);
        setShowAlert(true);
      }
    };
  ...
  return (
    <Container fluid className={`p-0 ${styles.latestProductsContainer}`}>
      {/* Alert for messages */}
      <Snackbar
        open={showAlert}
      ...
      <div className={styles.latestProductsHeader}>
        <h4 className=" mb-0 py-2">New Arrivals</h4>
      </div>
      <div className={styles.latestProductsList}>
        <Row className={`flex-nowrap overflow-auto ${styles.latestProductsRow}`}>
