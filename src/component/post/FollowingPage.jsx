...
import { FaArrowLeft, FaUserCheck, FaUserPlus, FaUsers } from "react-icons/fa";
  ...
  const fetchFollowing = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const followingList = (response.followingList || []).map(follow => {
          // Handle different possible data structures
          const user = follow.following || follow.user || follow;
          
          if (!user || !user.id) {
            console.warn('Invalid following data:', follow);
            return null;
          }
      ...
      <Row className="justify-content-center">
        <Col lg={8} xl={6}>
          ...
          <div className="d-flex align-items-center mb-4">
            ...
            <div>
              <h2 className="fw-bold mb-1">Following</h2>
              <p className="text-muted mb-0">{following.length} following</p>
            </div>
          </div>
            ...
            <Card.Body className="p-0">
              {following.length === 0 ? (
                <div className="text-center py-5">
                  ...
                  <p className="text-muted">When you follow someone, they'll appear here.</p>
                </div>
              ) : (
